...
const MDXComponents = {
  ..._MDXComponents,
  a: function MDXAnchor(props: React.ComponentPropsWithoutRef<typeof _MDXComponents.a>) {
    const docs = usePluginData('docusaurus-plugin-content-docs') as DocsPluginData;
    ...
    for (const version of docs.versions) {
      if (doc = version.docs.find((doc) => doc.id === docId || `/${doc.id}` === docId)) {
        break;
      }
    }
  ...
  h2: function MDXHeading2({ className, ...props }: React.ComponentPropsWithoutRef<typeof _MDXComponents.h2>) {
    return (
      <_MDXComponents.h2
        {...props}
...
function useChangelogContent(component: (props: MDXProps) => React.ReactElement<ChildProps>): React.ReactNode {
  ...
  const contentNodes = Children.toArray(root.props.children).filter(isValidElement<ChildProps>);
  const idxFirstH2 = contentNodes.findIndex(node => node.type === MDXComponents.h2);
  const mainDescriptions = contentNodes.slice(1, idxFirstH2);
  const idxAllH2 = contentNodes.map((node, i) => node.type === MDXComponents.h2 ? i : null).filter(Boolean);
  const versionSections = idxAllH2.reduce((sections, idx, iArr, arr) => {
    const section = contentNodes.slice(idx, arr[iArr + 1] ?? contentNodes.length);

    const dateString = (section[1].props.children as unknown as string).replace(/released on /i, '').slice(0, -1);
        ...
        {zippedSections.map(([toc, section], i) => (
          ...
          >
            <div className={styles.versionTimeline}>
              ...
              >
                <span>{toc.releaseDate.toFormat('DD', { locale: currentLocale })}</span>
              </time>
            </div>
